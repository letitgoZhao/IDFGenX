...
- [x] 1. 先增加目录契约和 README 内容的失败测试；
- [x] 2. 运行测试并确认旧路径/缺失 README 导致预期失败；
- [x] 3. 验证旧目录绝对路径和新目录不存在后执行单次目录迁移；
- [x] 4. 使用 `apply_patch` 新增中文 README；
- [x] 5. 更新当前文档引用，并给历史任务/报告增加迁移说明；
- [x] 6. 验证 68/68 副本哈希、32/32 核心转换和目录契约；
- [x] 7. 运行现有全量单测和 Python 编译；
- [x] 8. 检查 diff、敏感文件和非预期生成物；
- [x] 9. 输出执行报告并更新任务与 `STATUS.md`。
...
- [x] `data/selected_official_idfs` 是唯一规范目录，旧目录不存在；
- [x] README 简洁说明版本、五类目录作用、选择理由和限制；
- [x] 68 个 IDF 和 manifest 数据结构保持不变；
- [x] 68/68 哈希、32/32 转换与全量单测通过；
- [x] 当前引用已迁移，历史记录具有迁移说明；
