...
- [x] 1. 先为采样配置载入、校验和哈希写失败测试；
- [x] 2. 固定 NumPy/SciPy 依赖并实现不可变配置模型；
- [x] 3. 先为 LHS/Sobol 单桶采样、分层和确定性写失败测试；
- [x] 4. 实现候选生成、组合门禁、拒绝统计和结构化错误；
- [x] 5. 先为训练 40%/60% 配额和 C5 隔离写失败测试；
- [x] 6. 实现训练目录分配、子 seed、合并排序和 C5 OOD 门禁；
- [x] 7. 运行局部与全量验证，检查 diff、锁文件和敏感/运行时文件；
- [x] 8. 写执行报告，更新任务、`MASTER_PLAN.md` 和 `STATUS.md`。
...
- [x] 两种 QMC 引擎对相同 seed 可复现且不修改全局 RNG；
- [x] 离散字段分层，连续字段和所有返回 Spec 通过范围与组合门禁；
- [x] 训练批次达到精确数量及 40%/60% 配额，并排除 C5；
- [x] 显式 C5 每条均位于训练包络外；
- [x] 配置、公共接口、错误和追溯元数据有类型、中文 docstring 与测试；
- [x] 全量验证、锁检查和 diff 检查通过；
- [x] 无敏感数据或采样运行产物；
- [x] 报告、状态和任务记录完成。
