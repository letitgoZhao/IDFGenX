...
- [ ] 4. 编写实施计划；
- [ ] 5. 新增 ADR 并同步全部受影响文档；
- [ ] 6. 检查模型、硬件、精度、任务依赖和评估口径的一致性；
- [ ] 7. 运行文档、diff、敏感文件和 Git 状态检查；
- [ ] 8. 写执行报告，更新任务状态并提交、推送。
...
- [ ] 0.6B、1.7B、8B+ 的职责和晋级门明确；
- [ ] RTX 4060 8GB 仅作经 smoke 验证的本地训练承诺；
- [ ] 4B 不再是固定生产门，仍可作为可选中间对照；
- [ ] M3 同基座公平比较和 M5 最终部署出口同步更新；
- [ ] 未引入量化训练、模型权重或运行时产物；
- [ ] 报告已生成，`STATUS.md` 与 `MASTER_PLAN.md` 已更新。
